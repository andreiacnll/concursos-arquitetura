...
function normalizarAnalise(valor: unknown): Analise {
  ...
  return {
    ...
    stale_reasons: Array.isArray(item.stale_reasons)
      ? item.stale_reasons.map(String)
      : [],
    analysis_scope: item.analysis_scope ?? null,
    source_platform: item.source_platform ?? null,
    source_documents_count: Number(item.source_documents_count ?? 0),
    concurso_encerrado: Boolean(item.concurso_encerrado),
    link: item.link ?? null,
            ...
            return (
              <div key={`${analise.tipo ?? "job"}-${analise.id}`} className="analise-item">
                <div>
                  ...
                  <div className="analise-meta-badges">
                    {analise.stale && (
                      <span className="analise-meta-badge is-stale">
                        Desatualizada
                      </span>
                    )}
                    {analise.concurso_encerrado && (
                      <span className="analise-meta-badge">
                        Concurso encerrado
                      </span>
                    )}
                    {analise.analysis_scope === "sistema" && (
                      <span className="analise-meta-badge">
                        Histórica
                      </span>
                    )}
                    {analise.source_platform && (
                      <span className="analise-meta-badge">
                        {analise.source_platform} · {analise.source_documents_count ?? 0} docs
                      </span>
                    )}
                  </div>
                  {analise.recommendation && (
                    <p className="analise-recommendation">
                      Recomendação: {analise.recommendation}
                    </p>
                  )}
                  {analise.stale_reasons?.length ? (
                    <p className="analise-stale-reason">
                      {analise.stale_reasons[0]}
                    </p>
                  ) : null}
                </div>
