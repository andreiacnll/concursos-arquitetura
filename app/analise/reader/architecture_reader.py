...
def _read_source(path: Path, display_name: str) -> tuple[str, dict[str, Any] | None]:
...
def _source_texts(
    ...
    for item in manifest.items:
        if not item.accepted_for_reader:
            continue
        path = root / item.path
        text, structured = _read_source(path, item.path)
        if text:
...
def _find_money(text: str) -> list[str]:
    values = re.findall(
...
def _find_percentages(text: str) -> list[str]:
    values = re.findall(r"\b\d{1,3}\s*%", text)
    seen = []
    for value in values:
        clean = _clean(value)
        if clean not in seen:
...
def read_architecture_documents(
    *,
    ...
    source_texts, read_status, spreadsheet_results = _source_texts(manifest, root)
    ...
    prices = _find_money(combined)
    percentages = _find_percentages(combined)
    ...
    result = {
        ...
        "post_award_documents": [
            ...
            for line in _lines_matching(combined, ["habilitação", "habilitacao", "adjudicatário"], 10)
        ],
        "financial_conditions": {
            "payments": [
                _evidence(line, document=first_document, section="Pagamentos")
                for line in _lines_matching(combined, ["pagamento", "preço", "preco", "caução"], 10)
            ]
        },
        "exclusion_risks": [
