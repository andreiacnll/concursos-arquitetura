...
DIAS_A_PESQUISAR = int(os.getenv("BASE_DIAS_PESQUISA", "2"))
MAXIMO_PAGINAS = int(os.getenv("BASE_MAX_PAGINAS", "100"))
MAXIMO_DETALHES = int(os.getenv("BASE_MAX_DETALHES", "300"))
INTERVALO_PEDIDOS = float(os.getenv("BASE_INTERVALO_PEDIDOS", "0.3"))
INTERVALO_DETALHES = float(os.getenv("BASE_INTERVALO_DETALHES", "0.25"))
    ...
    def post_json(self, dados: dict[str, Any]) -> dict[str, Any]:
        ...
        if estado >= 400:
            raise RuntimeError(
                ...
                f"HTTP {estado} em {resultado.get('url', URL_RESULTADOS)}. "
                f"Excerto: {texto[:500]}"
            )
        ...
        try:
            ...
            raise RuntimeError(
                "O Portal BASE devolveu uma resposta que não é JSON. "
                f"HTTP {estado}; content-type="
                f"{resultado.get('contentType', '')}; excerto={texto[:500]}"
            ) from erro
...
def enriquecer(
    ...
    candidatos = [
        anuncio for anuncio in listagem if deve_consultar_detalhe(anuncio)
    ]
    ...
    for indice, candidato in enumerate(candidatos, start=1):
        identificador = normalizar_texto(candidato.get("id"))
        ...
        if parece_relevante(completo):
            resultados.append(normalizar_anuncio(completo))

        if INTERVALO_DETALHES > 0:
            time.sleep(INTERVALO_DETALHES)
