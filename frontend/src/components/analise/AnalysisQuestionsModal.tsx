...
function teamRoleKey(item: AnyRecord): string {
  const target = item?.profile_target;
  return folded(
    target && typeof target === "object"
      ? target.role ?? item?.role ?? item?.title ?? item?.label
      : item?.role ?? item?.title ?? item?.label,
  );
}

function requiredSubmissionTeamRoles(procedure: AnyRecord): string[] {
  const team = Array.isArray(procedure?.technical_team)
    ? procedure.technical_team
    : [];
  const roles = team
    .filter((item: AnyRecord) => item && typeof item === "object")
    .map(teamRoleKey)
    .filter(Boolean);
  const explicit = team
    .filter((item: AnyRecord) => item?.required_at_submission === true)
    .map(teamRoleKey)
    .filter(Boolean);
  ...
  const documents = team
    .map((item: AnyRecord) => clean(item?.source_document))
    .join(" ");
  const headings = team
    .map((item: AnyRecord) => clean(item?.source_heading))
    .join(" ");
  const uniqueRoles = [...new Set(roles)];

  // Legacy compatibility: a complete team from the Programme is sufficient evidence.
  // It remains valid before the canonical fields are available.
  if (
    uniqueRoles.length >= 3 &&
    folded(documents).includes("programa") &&
    (folded(headings).includes("equipa") || folded(headings).includes("anexo"))
  ) {
    return uniqueRoles;
  }
...
function canonicalCoversSubmissionTeam(
  canonical: AnyRecord,
  procedure: AnyRecord,
): boolean {
  const expected = requiredSubmissionTeamRoles(procedure);
  ...
  const covered = new Set(
    requirements
      .filter(
        (item: AnyRecord) =>
          item?.nature === "team" &&
          item?.stage === "pre_award" &&
          item?.profile_dependent === true &&
          item?.required_at_submission !== false,
      )
      .map(teamRoleKey)
      .filter(Boolean),
  );

  return expected.every((role) => covered.has(role));
}

export function canonicalNeedsEnsure(ficha: AnyRecord): boolean {
  ...
  const missingSubmissionTeamCoverage = procedures.some(
    (procedure) =>
      procedure &&
      typeof procedure === "object" &&
      !canonicalCoversSubmissionTeam(canonical, procedure),
  );

  // This terminal status is valid only while the visible analysis has no procedural evidence.
  // It remains valid before the canonical fields are available.
  // com requisitos vazios mesmo tendo equipa exigida com a proposta.
  if (recoveryStatus === "no_procedural_evidence") {
    return missingSubmissionTeamCoverage;
  }
  ...
  if (missingSubmissionTeamCoverage) return true;
...
export default function AnalysisQuestionsModal({
  ...
  return (
    <>
      {inlineTriggerLabel ? <button type="button" className="aqm-inline-trigger" onClick={openEditor}>{pendingQuestions.length ? `${inlineTriggerLabel} · ${pendingQuestions.length}` : inlineTriggerLabel}</button> : null}
      ...
      <style jsx global>{`
        .aqm-inline-trigger { display: inline-flex; align-items: center; gap: 7px; margin: 0 0 16px; padding: 10px 13px; border: 1px solid #cbd8b8; border-radius: 9px; background: #f5f8ef; color: #536b35; font-size: 12px; font-weight: 800; cursor: pointer; }
        .aqm-inline-trigger:hover { background: #eaf1dc; }
